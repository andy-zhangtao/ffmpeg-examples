...
- **非商业用途**：本项目不用于任何商业目的
- **学习示例**：代码仅作为 FFmpeg API 学习参考
- **用户责任**：使用者需自行确保商业使用时的许可证合规性
- **依赖声明**：本项目依赖的 FFmpeg 可能包含 GPL 组件

### 商业使用警告
如果您计划将本项目代码用于商业目的，请：
1. 检查您的 FFmpeg 配置：`ffmpeg -version | grep configuration`
2. 如发现 `--enable-gpl`，请使用 LGPL 兼容版本或获取商业许可
3. 咨询法律顾问确保合规性

本项目作者不承担任何因许可证不合规导致的法律责任。
