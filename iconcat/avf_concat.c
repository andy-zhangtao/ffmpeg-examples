...
typedef struct ConcatContext {
    ...
    boolean_t b_in_split_status;
    boolean_t b_in_insert_status;
    boolean_t b_split_status[];
} ConcatContext;
...
static av_cold int init(AVFilterContext *ctx) {
    ...
    for (int i = 0; i < ctx->nb_inputs; i++) {
        cat->b_split_status[i] = false;
    }

    cat->b_in_split_status = false;
    cat->b_in_insert_status = false;
...
static int activate(AVFilterContext *ctx) {
    ...
    if (cat->cur_idx < ctx->nb_inputs) {
        ...
        for (i = 0; i < ctx->nb_outputs; i++) {
            ...
            if (ret < 0)
                return ret;
            if (ret) {
                if (cat->b_in_insert_status) {
                    ff_filter_set_ready(ctx, 10);
                    return push_frame(ctx, cat->cur_idx + i, frame);
                }

                int64_t time = frame->pts * av_q2d(ctx->inputs[cat->cur_idx + 1]->time_base);
                if (time >= 5 && !cat->b_in_split_status) {
                    /** 需要处理 */
                    cat->b_in_split_status = true; /** 进入暂存状态 */
                    cat->b_in_insert_status = true;
                    cat->b_split_status[i] = true; /** 指定输入源处于暂存状态 */
                }
                ff_filter_set_ready(ctx, 10);
                return push_frame(ctx, cat->cur_idx + i, frame);
            }
    ...
    if (cat->cur_idx < ctx->nb_inputs) {
        for (i = 0; i < ctx->nb_outputs; i++) {
            /**
             * 检查当前的输入流是否还有未处理的帧, 如果没有了，就关闭此输入流
             * */

            ret = ff_inlink_acknowledge_status(ctx->inputs[cat->cur_idx + i], &status, &pts);
            /* TODO use pts */
            av_log(ctx, AV_LOG_VERBOSE, "ACKNOWLEDGE IDX: %d RET: %d NB_IN_ACTIVE: %d PTS: %lld TIME: %s STATUS: %d \n",
                   cat->cur_idx, ret,
                   cat->nb_in_active, pts, av_ts2timestr(pts, &ctx->inputs[cat->cur_idx]->time_base), status);

            if (cat->b_in_split_status) {
                for (int i = 0; i < ctx->nb_inputs; i++) {
                    if (cat->b_split_status[i]) {
                        cat->cur_idx = i + 1;
                        cat->b_in_split_status = false;
                    }
                }
            ...
            if (ret > 0) {
                ...
                if (cat->b_in_insert_status && cat->cur_idx == 0 && status < 0) {
                ...
                ff_filter_set_ready(ctx, 10);
                if (cat->b_in_insert_status && !cat->b_in_split_status && cat->cur_idx > 0) {
                    /** 第二个视频处理完毕, 继续处理第一个视频 */
                    cat->cur_idx = cat->cur_idx - 2;
                    cat->b_in_split_status = false;
                    cat->b_in_insert_status = true;
                }
                return 0;
